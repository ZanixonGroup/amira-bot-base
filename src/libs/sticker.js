...
async function writeExif(media, metadata) {
   ...
   if (Object.keys(metadata).length != 0 || Object.keys(global?.exif).length != 0) {
      ...
      let opt = { packId: metadata?.packId ? metadata.packId : global?.exif?.packId, packName: metadata?.packName ? metadata.packName : '', packPublish: metadata?.packPublish ? metadata.packPublish : '', packEmail: metadata?.packEmail ? metadata.packEmail : global?.exif?.packEmail, packWebsite: metadata?.packWebsite ? metadata.packWebsite : global?.exif?.packWebsite, androidApp: metadata?.androidApp ? metadata.androidApp : global?.exif?.androidApp, iOSApp: metadata?.iOSApp ? metadata.iOSApp : global?.exif?.iOSApp, emojis: metadata?.emojis ? metadata.emojis : global?.exif?.emojis, isAvatar: metadata?.isAvatar ? metadata.isAvatar : global?.exif?.isAvatar }
